 ...
 * @access public
 */
class ButterBean_Event_Control_File extends ButterBean_Event_Control {
    ...
    public function __construct( $manager, $name, $args = array() ) {
        ...
        $this->l10n = wp_parse_args(
            $this->l10n,
            array(
                'upload'      => esc_html__( 'Add pdf',         'stm_motors_events' ),
                'set'         => esc_html__( 'Set as pdf',      'stm_motors_events' ),
                'choose'      => esc_html__( 'Choose pdf',      'stm_motors_events' ),
                'change'      => esc_html__( 'Change pdf',      'stm_motors_events' ),
                'remove'      => esc_html__( 'Remove pdf',      'stm_motors_events' ),
                'placeholder' => esc_html__( 'No pdf selected', 'stm_motors_events' )
            )
        );
    }
