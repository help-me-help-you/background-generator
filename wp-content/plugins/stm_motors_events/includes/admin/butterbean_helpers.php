...
add_action('plugins_loaded', 'stm_motors_events_load_butterbean');
function stm_motors_events_load_butterbean()
{
    require_once(STM_EVENTS_PATH . '/includes/admin/butterbean/butterbean.php');
}

function stm_motors_events_validate_checkbox($value)
{
    return wp_validate_boolean($value) ? 'on' : false;
}

function stm_motors_events_no_validate($value)
{
    return $value;
}

function stm_motors_events_validate_image($value)
{
    return !empty($value) ? intval($value) : false;
}

function stm_motors_events_validate_repeater($value, $butterbean)
{
...
function stm_motors_events_multiselect($value, $butterbean)
{
    global $post;
    $post_id = $post->ID;
...
function stm_motors_events_validate_gallery($value)
{
    $value = explode(',', $value);
    $values = array();
...
function stm_motors_events_date($value) {
...
function stm_motors_events_get_user_list()
{
    $users_args = array(
    ...
    $users_dropdown = array(
        '' => esc_html__('Not assigned', 'stm_motors_events')
    );
...
function stm_motors_events_add_category_in()
{
    $response = array();
    $category = $term = '';
    ...
    if (!empty($term) and !empty($category)) {
        ...
        if ($term_id === 0 or $term_id === null) {
            ...
            $response['message'] = sprintf(esc_html__('%s already added!', 'stm_motors_events'), $term_info->name);
        ...
        if (!empty($term_id) and !is_wp_error($term_id)) {
            $term_info = get_term_by('id', $term_id['term_id'], $category);
            $response['message'] = sprintf(esc_html__('%s added!', 'stm_motors_events'), $term_info->name);
            $response['status'] = 'success';
            $response['slug'] = $term_info->slug;
            $response['name'] = $term_info->name;
        } else {
...
add_action('wp_ajax_stm_motors_events_add_category_in', 'stm_motors_events_add_category_in');