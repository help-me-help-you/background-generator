...
foreach($cars->get_posts() as $k => $val) {
    ...
    if(get_post_meta($val->ID, "stm_lat_car_admin") != null && $val->post_status == "publish" || $val->post_status == "private") {
        ...
        if(isset($carsInfo[0]) && isset($carMeta[$carsInfo[0]["key"]])) {
            $term = get_term_by('slug', $carMeta[$carsInfo[0]["key"]][0], $carsInfo[0]["key"]);
            if($term) {
                $mileage = $term->name;
            }
        }

        if(isset($carsInfo[1]) && isset($carMeta[$carsInfo[1]["key"]])) {
            $term = get_term_by('slug', $carMeta[$carsInfo[1]["key"]][0], $carsInfo[1]["key"]);
            if($term) {
                $engine = $term->name;
            }
        }

        if(isset($carsInfo[2]) && isset($carMeta[$carsInfo[2]["key"]])) {
            $term = get_term_by('slug', $carMeta[$carsInfo[2]["key"]][0], $carsInfo[2]["key"]);
            if($term) {
                $transmission = $term->name;
            }
        }
