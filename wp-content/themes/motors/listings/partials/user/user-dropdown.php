<?php if(is_user_logged_in()): ?>
	<?php
		...
		$user_cars = (function_exists('stm_user_listings_query')) ? stm_user_listings_query($user->data->ID) : null;
	...
	<div class="lOffer-account-dropdown stm-login-form-unregistered">
		<form method="post">
			...
			<div class="form-group">
				...
				<input type="text" name="stm_user_login" autocomplete="off" placeholder="<?php esc_html_e('Enter login or E-mail', 'motors') ?>"/>
			</div>

			<div class="form-group">
				<h4><?php esc_html_e('Password', 'motors'); ?></h4>
				<input type="password" name="stm_user_password" autocomplete="off" placeholder="<?php esc_html_e('Enter password', 'motors') ?>"/>
			</div>
            ...
            <?php if(function_exists('icl_object_id')) : ?><input type="hidden" name="current_lang" value="<?php echo ICL_LANGUAGE_CODE; ?>"/><?php endif; ?>
