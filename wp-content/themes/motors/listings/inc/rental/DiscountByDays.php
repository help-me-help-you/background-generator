...
class DiscountByDays
{
    ...
    {
        add_action('add_meta_boxes', array(get_class(), 'addDiscountByDaysMetaBox'));
        add_action('save_post', array(get_class(), 'add_days_post_meta'), 10, 2);
        add_filter( 'woocommerce_product_get_price', array(get_class(), 'updateVariationPriceWithDiscount'), 30, 2);
        add_filter( 'woocommerce_product_variation_get_price', array(get_class(), 'updateVariationPriceWithDiscount'), 30, 2);
        add_filter( 'stm_cart_items_content', array(get_class(), 'updateCart'), 40, 1);
    }

    public static function addDiscountByDaysMetaBox() {
        add_meta_box(
            'discount_by_days_repitor'
            ,__( 'Discount By Days', 'motors' )
            ,array(get_class(), 'discountByDaysView')
            ,'product'
            ,'advanced'
            ,'high'
        );
    ...
    public static function getPercent ($varId) {
        ...
        if($orderCookieData['pickup_date'] != '--' && $orderCookieData['return_date'] != '--') {
            $date1 = new DateTime($orderCookieData['pickup_date']);
            $date2 = new DateTime($orderCookieData['return_date']);
            ...
            <ul class="discount-by-days-list">
                ...
                if(!empty($periods)) :
                    ...
                    foreach ($periods as $k => $val) : ?>
                        <li>
                            <div class="repeat-days-number"><?php echo $i;?></div>
                            <table>
                                <tr>
                                    <td>
                                        Days
                                    ...
                                    <td>
                                        <input type="text" value="<?php echo $val['days']; ?>" name="days[]" />
                                    </td>
                                    <td>>=</td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" value="<?php echo $val['percent']; ?>" name="percent[]" />
                                    </td>
                                    <td>
                                        %
                                    </td>
