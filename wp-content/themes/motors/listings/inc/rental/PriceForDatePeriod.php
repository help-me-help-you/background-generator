...
class PriceForDatePeriod {
    ...
    public function __construct()
    {
        ...
        add_action('add_meta_boxes', array(get_class(), 'addPriceForDateMetaBox'));
        add_action( 'admin_enqueue_scripts', array(get_class(), 'loadScriptStyles') );
        add_action('save_post', array(get_class(), 'stm_save_customer_note_meta'), 10, 2);
        add_filter( 'woocommerce_product_get_price', array(get_class(), 'updateVariationPrice'), 20, 2);
        add_filter( 'woocommerce_product_variation_get_price', array(get_class(), 'updateVariationPrice'), 20, 2);
        add_filter( 'stm_cart_items_content', array(get_class(), 'updateCart'), 30, 1);
    }
    ...
    public static function updateVariationPrice ($price, $product) { // update total cart
        ...
        $dates = getDateRangeWithTime($orderCookieData['pickup_date'], $orderCookieData['return_date']);
                ...
                foreach ($cart as $cart_item) {
                    ...
                    if ($buy_type == 'options') {
                        ...
                        $cart_items[$buy_type][] = array(
                            ...
                            'name' => $post->get_title(),
                            'price' => getDefaultVariablePrice($id),
    ...
    public static function loadScriptStyles () {
        wp_enqueue_script( 'rental-price-js', get_template_directory_uri() . '/inc/rental/assets/js/rental-price.js' );
    ...
    public static function addPriceForDateMetaBox() {
        add_meta_box(
            'rental_price_for_date_repitor'
            ,__( 'Price for Date Period', 'motors' )
            ,array(get_class(), 'priceForDateView')
            ,'product'
            ,'advanced'
            ,'high'
        );
    ...
    public function getVariationPriceView ($carId) {
        $table_name = self::$wpdbObj->prefix. "rental_price_date";
            ...
            <ul class="rental-price-date-list">
                ...
                if(!empty($periods)) :
                    foreach ($periods as $k => $val) : ?>
                        <li>
                            <div class="repeat-number"><?php echo $k+1;?></div>
                            <table>
                                <tr>
                                    <td>
                                        Pickup Date
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" value="<?php echo $val->price; ?>" name="date-price[]" />
                                    </td>
                                </tr>
                            </table>
                            <div class="btn-wrap">
                                <button class="remove-fields button-secondary" data-remove="<?php echo $val->id; ?>">Remove</button>
                            </div>
                        </li>
                    <?php
                    endforeach;
