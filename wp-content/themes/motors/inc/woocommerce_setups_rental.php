...
function getDateRangeWithTime($date1, $date2) {
...
function remove_order_custom_post_meta_hard($order_id) {
    global $wpdb;
    $metaKeys = $wpdb->get_results("SELECT post_id, meta_value FROM " . $wpdb->prefix . "postmeta WHERE meta_key = 'order_meta_dates_" . $order_id . "'");
...
add_action( 'woocommerce_order_status_completed', 'remove_order_custom_post_meta_hard', 100, 1);
add_action( 'woocommerce_order_status_cancelled', 'remove_order_custom_post_meta_hard', 100, 1);
add_action('wp_trash_post', 'remove_order_custom_post_meta_hard', 100, 1);

function getDefaultVariablePrice ($prodId, $index = 0, $from = false) {
    $price = get_post_meta($prodId, '_price');
    if(!$from) rsort($price);
    return $price[$index];
}

function getInfoWindowPriceManip($id) {
    $priceDate = new PriceForDatePeriod();
    $priceDate = $priceDate->getVariationPriceView($id);

    $discount = new DiscountByDays();
    $discount = $discount->get_days_post_meta($id);
...
add_filter('product_type_options', 'wc_hook_stm_product_type_options');

function wc_hook_stm_product_type_options($checkboxes)
{
